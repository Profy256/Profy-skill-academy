...
public class GeminiProvider implements LLMProvider {
    ...
    public ChatResponse complete(List<ChatMessage> messages, String systemPrompt,
        ...
        try {
            ...
            log.error("Gemini unreachable: {}", e.getMessage());
            throw new AiUnavailableException("Gemini is temporarily unavailable",
                    AiUnavailableException.Reason.NETWORK);
        } catch (AiUnavailableException e) {
            throw e;
        } catch (HttpClientErrorException.Unauthorized | HttpClientErrorException.Forbidden e) {
            log.warn("{} rejected API key ({})", "Gemini", e.getStatusCode().value());
            throw new AiUnavailableException("API key rejected by Gemini",
                    AiUnavailableException.Reason.AUTH);
        } catch (HttpClientErrorException.TooManyRequests e) {
            log.warn("{} rate limited", "Gemini");
            throw new AiUnavailableException("Gemini rate limit reached",
                    AiUnavailableException.Reason.RATE_LIMIT);
        } catch (HttpClientErrorException e) {
            log.error("{} client error: {}", "Gemini", e.getStatusCode());
            throw new AiUnavailableException("Gemini returned status " + e.getStatusCode().value(),
                    AiUnavailableException.Reason.OTHER);
        } catch (HttpServerErrorException e) {
            log.error("{} server error: {}", "Gemini", e.getStatusCode());
            throw new AiUnavailableException("Gemini is having server trouble",
                    AiUnavailableException.Reason.NETWORK);
        } catch (Exception e) {
            log.error("{} error: {}", "Gemini", e.getMessage());
            throw new AiUnavailableException("Gemini encountered an error",
                    AiUnavailableException.Reason.OTHER);
        }
    }
}
